((a,b)=>{a[b]=a[b]||{}})(self,"$__dart_deferred_initializers__")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,E,B={
c9B(){return new B.yy(null)},
yy:function yy(d){this.a=d},
aGm:function aGm(d){this.a=d},
aGn:function aGn(d,e){this.a=d
this.b=e},
aGo:function aGo(d,e){this.a=d
this.b=e},
amK:function amK(d,e,f,g,h){var _=this
_.c=d
_.d=e
_.e=f
_.f=g
_.a=h},
l2:function l2(d,e,f,g){var _=this
_.a=d
_.b=e
_.c=f
_.d=g}},D,F
A=c[0]
C=c[2]
E=c[17]
B=a.updateHolder(c[4],B)
D=c[33]
F=c[18]
B.yy.prototype={
p(d){var x,w,v,u,t,s,r=null,q=y.y.a(A.e(d).c.h(0,A.j(y.D)))
q.toString
x=A.az(d).a
w=y.u
v=A.a([A.Gm(C.aa,new B.aGm(x),r,r,r,r,r,r,"Print",r,"Enable or disable the entities you want to manage in your factory. Configure details for each enabled entity.","Manage Entities to Track",!0),A.d(r,q.at,r)],w)
for(q=q.Q,u=0;u<12;++u){t=A.a([],w)
if(u>0)t.push(new A.aR(r,q,r,r))
s=D.wH[u]
t.push(new B.amK(s,!x.f.n(0,s.a),new B.aGn(x,u),new B.aGo(x,u),r))
C.f.t(v,t)}return A.t(v,C.v,C.n,C.p,0,C.q)}}
B.amK.prototype={
p(d){var x,w,v,u,t,s,r,q=this,p=null,o=y.F.a(A.e(d).c.h(0,A.j(y.C)))
o.toString
x=y.y.a(A.e(d).c.h(0,A.j(y.D)))
x.toString
w=x.at
v=A.ad(x.d)
u=A.aO(o.ch,1)
t=q.c
s=t.a
s=A.qo(A.mZ(d,s),F.KF(d,s),t.d,x.k4,p,40)
r=q.d
x=A.d(p,x.Q,p)
return A.a5(p,A.t(A.a([new E.jQ(s,t.b,t.c,r,q.e,p),x,new A.cG(C.ek,p,p,A.aA(p,!1,C.G,p,p,!1,"Configure",p,p,r?q.f:p,p,C.a6,p,C.aa,120),p)],y.u),C.v,C.n,C.p,0,C.q),C.y,p,p,new A.aa(o.r,p,u,v,p,p,C.H),p,p,p,p,new A.H(w,w,w,w),p,p,p)}}
B.l2.prototype={}
var z=a.updateTypes([])
B.aGm.prototype={
$0(){return this.a.hz(0,"settings")},
$S:0}
B.aGn.prototype={
$1(d){return this.a.alD(D.wH[this.b].a,!d)},
$S:3}
B.aGo.prototype={
$0(){var x,w=D.wH[this.b],v=this.a.y
if(v!=null){x=y.w
v.akR("settings-entity-configure",A.ah(["entityKey",w.a],x,x))}return null},
$S:0};(function inheritance(){var x=a.inheritMany,w=a.inherit
x(A.M,[B.yy,B.amK])
x(A.kn,[B.aGm,B.aGo])
w(B.aGn,A.it)
w(B.l2,A.P)})()
A.lX(b.typeUniverse,JSON.parse('{"yy":{"M":[],"f":[],"i":[]},"amK":{"M":[],"f":[],"i":[]}}'))
var y={C:A.a8("e9"),D:A.a8("ey"),u:A.a8("u<f>"),w:A.a8("c"),F:A.a8("e9?"),y:A.a8("ey?")};(function constants(){var x=a.makeConstList
D.bBk=new B.l2("people","People","Track employees, contractors, and visitors in your factory",C.dF)
D.bBh=new B.l2("machines","Machines","Monitor and manage industrial equipment and machinery",C.iW)
D.bBf=new B.l2("tools","Tools","Track hand tools, measuring equipment, and other instruments",C.iT)
D.bBe=new B.l2("materials","Materials","Manage raw materials, components, and finished goods inventory",C.ex)
D.bBl=new B.l2("workingAreas","Working Areas","Organize departments, floors, zones, and production areas",C.kO)
D.bBd=new B.l2("locations","Locations","Define physical locations and facilities within your factory",C.fM)
D.bBm=new B.l2("devices","Devices","Manage IoT devices, sensors, and connected equipment",C.kQ)
D.bBj=new B.l2("clients","Clients","Track client accounts, contacts, and engagement history",C.nU)
D.bBg=new B.l2("workstations","Workstations","Manage operator workstations and shop-floor terminals",C.kP)
D.bBc=new B.l2("projects","Projects","Plan and track ongoing manufacturing projects and milestones",C.kN)
D.bBi=new B.l2("software","Software","Manage software licenses, applications, and tooling",C.dw)
D.bBb=new B.l2("products","Products","Catalog finished products, variants, and SKUs",C.ex)
D.wH=x([D.bBk,D.bBh,D.bBf,D.bBe,D.bBl,D.bBd,D.bBm,D.bBj,D.bBg,D.bBc,D.bBi,D.bBb],A.a8("u<l2>"))})()};
(a=>{a["2W1fPtCG0azRMjzAOuCUaxBrlMs="]=a.current})($__dart_deferred_initializers__);