((a,b)=>{a[b]=a[b]||{}})(self,"$__dart_deferred_initializers__")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,E,B={
ca5(){return new B.yx(null)},
yx:function yx(d){this.a=d},
aGt:function aGt(d){this.a=d},
aGu:function aGu(d,e){this.a=d
this.b=e},
aGv:function aGv(d,e){this.a=d
this.b=e},
amT:function amT(d,e,f,g,h){var _=this
_.c=d
_.d=e
_.e=f
_.f=g
_.a=h},
l2:function l2(d,e,f,g){var _=this
_.a=d
_.b=e
_.c=f
_.d=g}},D,F
A=c[0]
C=c[2]
E=c[17]
B=a.updateHolder(c[4],B)
D=c[33]
F=c[18]
B.yx.prototype={
p(d){var x,w,v,u,t,s,r=null,q=y.y.a(A.e(d).c.h(0,A.j(y.D)))
q.toString
x=A.az(d).a
w=y.u
v=A.a([A.Gr(C.a9,new B.aGt(x),r,r,r,r,r,r,"Print",r,"Enable or disable the entities you want to manage in your factory. Configure details for each enabled entity.","Manage Entities to Track",!0),A.d(r,q.at,r)],w)
for(q=q.Q,u=0;u<12;++u){t=A.a([],w)
if(u>0)t.push(new A.aR(r,q,r,r))
s=D.wH[u]
t.push(new B.amT(s,!x.f.n(0,s.a),new B.aGu(x,u),new B.aGv(x,u),r))
C.f.t(v,t)}return A.t(v,C.v,C.n,C.p,0,C.q)}}
B.amT.prototype={
p(d){var x,w,v,u,t,s,r,q=this,p=null,o=y.F.a(A.e(d).c.h(0,A.j(y.C)))
o.toString
x=y.y.a(A.e(d).c.h(0,A.j(y.D)))
x.toString
w=x.at
v=A.ad(x.d)
u=A.aP(o.ch,1)
t=q.c
s=t.a
s=A.qq(A.mY(d,s),F.KL(d,s),t.d,x.k4,p,40)
r=q.d
x=A.d(p,x.Q,p)
return A.a3(p,A.t(A.a([new E.jP(s,t.b,t.c,r,q.e,p),x,new A.cG(C.el,p,p,A.ay(p,!1,C.G,p,p,!1,"Configure",p,p,r?q.f:p,p,C.a6,p,C.a9,120),p)],y.u),C.v,C.n,C.p,0,C.q),C.y,p,p,new A.ab(o.r,p,u,v,p,p,C.H),p,p,p,p,new A.H(w,w,w,w),p,p,p)}}
B.l2.prototype={}
var z=a.updateTypes([])
B.aGt.prototype={
$0(){return this.a.hA(0,"settings")},
$S:0}
B.aGu.prototype={
$1(d){return this.a.am_(D.wH[this.b].a,!d)},
$S:3}
B.aGv.prototype={
$0(){var x,w=D.wH[this.b],v=this.a.y
if(v!=null){x=y.w
v.ald("settings-entity-configure",A.ah(["entityKey",w.a],x,x))}return null},
$S:0};(function inheritance(){var x=a.inheritMany,w=a.inherit
x(A.M,[B.yx,B.amT])
x(A.kn,[B.aGt,B.aGv])
w(B.aGu,A.is)
w(B.l2,A.P)})()
A.lW(b.typeUniverse,JSON.parse('{"yx":{"M":[],"f":[],"i":[]},"amT":{"M":[],"f":[],"i":[]}}'))
var y={C:A.a8("e7"),D:A.a8("ey"),u:A.a8("u<f>"),w:A.a8("c"),F:A.a8("e7?"),y:A.a8("ey?")};(function constants(){var x=a.makeConstList
D.bBm=new B.l2("people","People","Track employees, contractors, and visitors in your factory",C.dF)
D.bBj=new B.l2("machines","Machines","Monitor and manage industrial equipment and machinery",C.iX)
D.bBh=new B.l2("tools","Tools","Track hand tools, measuring equipment, and other instruments",C.iU)
D.bBg=new B.l2("materials","Materials","Manage raw materials, components, and finished goods inventory",C.ey)
D.bBn=new B.l2("workingAreas","Working Areas","Organize departments, floors, zones, and production areas",C.kP)
D.bBf=new B.l2("locations","Locations","Define physical locations and facilities within your factory",C.fM)
D.bBo=new B.l2("devices","Devices","Manage IoT devices, sensors, and connected equipment",C.kR)
D.bBl=new B.l2("clients","Clients","Track client accounts, contacts, and engagement history",C.nU)
D.bBi=new B.l2("workstations","Workstations","Manage operator workstations and shop-floor terminals",C.kQ)
D.bBe=new B.l2("projects","Projects","Plan and track ongoing manufacturing projects and milestones",C.kO)
D.bBk=new B.l2("software","Software","Manage software licenses, applications, and tooling",C.dw)
D.bBd=new B.l2("products","Products","Catalog finished products, variants, and SKUs",C.ey)
D.wH=x([D.bBm,D.bBj,D.bBh,D.bBg,D.bBn,D.bBf,D.bBo,D.bBl,D.bBi,D.bBe,D.bBk,D.bBd],A.a8("u<l2>"))})()};
(a=>{a["0W+ysSQYURmn7D5XGEpD/xhQY8E="]=a.current})($__dart_deferred_initializers__);